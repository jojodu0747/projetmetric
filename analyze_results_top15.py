...
def create_top15_overall(df, output_dir):
    """Figure: Top 15 meilleures configurations (monotonie moyenne)."""
    # Trier par monotonie moyenne
    df_sorted = df.sort_values('monotonie_moyenne', ascending=False).head(15)
    ...
    total = len(df_sorted)
    stats_text = "Proportion dans Top 15:\n"
    for bb, count in backbone_counts.most_common():
        pct = 100 * count / total
        stats_text += f"  {bb}: {count}/15 ({pct:.1f}%)\n"
    ...
    ax1.set_title('Top 15 Meilleures Configurations (Monotonie Moyenne)', fontsize=14, fontweight='bold')
    ax1.set_xlim(0, 1.1)
    ax1.axvline(x=1.0, color='grey', linestyle='--', alpha=0.3)
    ax1.grid(axis='x', alpha=0.3)
    ax1.invert_yaxis()

    # Stats dans le panneau de droite
    ax2.axis('off')
    ax2.text(0.1, 0.95, stats_text, fontsize=11, verticalalignment='top',
    ...
    ax2.set_title('Répartition par Backbone', fontsize=12, fontweight='bold', pad=20)

    plt.tight_layout()
    output_path = f"{output_dir}/fig_top15_overall.png"
    plt.savefig(output_path, dpi=200, bbox_inches='tight')
    print(f"✓ Saved {output_path}")
    plt.close()


def create_top15_per_degradation(df, output_dir):
    """Figures: Top 15 par type de dégradation."""
    ...
    for deg_col in deg_cols:
        ...
        df_sorted = df.sort_values(deg_col, ascending=False).head(15)
        ...
        ax1.set_title(f'Top 15 Meilleures Configurations - {deg_name}', fontsize=14, fontweight='bold')
        ax1.set_xlim(0, 1.1)
        ax1.axvline(x=1.0, color='grey', linestyle='--', alpha=0.3)
        ax1.grid(axis='x', alpha=0.3)
        ax1.invert_yaxis()
        ...
        if backbone_counts:
            ...
            ax2.pie([count for _, count in backbone_counts.most_common()],
                   labels=pie_labels, colors=pie_colors, autopct='%1.1f%%',
                   startangle=90, textprops={'fontsize': 9})
            ax2.set_title(f'Répartition - {deg_name}', fontsize=12, fontweight='bold', pad=20)

        plt.tight_layout()
        output_path = f"{output_dir}/fig_top15_{deg_col.replace('mono_', '')}.png"
        plt.savefig(output_path, dpi=200, bbox_inches='tight')
        print(f"✓ Saved {output_path}")
        plt.close()
...
def main():
    parser = argparse.ArgumentParser(description="Analyse Top 15 avec stats")
    ...
    print(f"\n{'='*80}")
    print("ANALYSE TOP 15 + PROFILS SIMPLIFIÉS")
    print(f"{'='*80}\n")
    ...
    # Top 15 overall
    create_top15_overall(df, args.output_dir)

    # Top 15 per degradation
    create_top15_per_degradation(df, args.output_dir)
