...
BACKBONE_CONFIGS = {
    ...
    "lpips_vgg": {
        ...
        "normalize_mean": [0.5, 0.5, 0.5],
        "normalize_std": [0.5, 0.5, 0.5],
        "total_layers": 13,  # VGG16 has 13 Conv2d layers
        "layer_names": {
            ...
            7: "net.slice4.17",   # conv4_1 (512 ch) - CORRIGÉ
            8: "net.slice4.19",   # conv4_2 (512 ch)
            9: "net.slice4.21",   # conv4_3 (512 ch)
            10: "net.slice5.24",  # conv5_1 (512 ch) - CORRIGÉ
            11: "net.slice5.26",  # conv5_2 (512 ch) - CORRIGÉ
            12: "net.slice5.28",  # conv5_3 (512 ch) - CORRIGÉ
        },
    },
    "clip_vit_base": {
...
ENABLED_LAYERS = {
    "sd_vae": {
        ...
        1: True,  # encoder.down_blocks.0.resnets.0 - ResNet block (128ch, 256×256)
        ...
        5: True,  # encoder.down_blocks.1.resnets.1 - ResNet block (256ch, 128×128)
        6: False,   # encoder.down_blocks.1.downsamplers.0 - Downsample (256ch→64×64, mid)
        7: True,  # encoder.down_blocks.2.resnets.0 - ResNet block (512ch, 64×64)
        8: True,  # encoder.down_blocks.2.resnets.1 - ResNet block (512ch, 64×64)
        9: False,   # encoder.down_blocks.2.downsamplers.0 - Downsample (512ch→32×32, mid-late)
        10: True, # encoder.down_blocks.3.resnets.0 - ResNet block (512ch, 32×32)
        11: True, # encoder.down_blocks.3.resnets.1 - ResNet block (512ch, 32×32)
        12: False,  # encoder.mid_block.resnets.0 - Bottleneck ResNet (512ch, 32×32, late)
        13: True, # encoder.mid_block.attentions.0 - Self-attention (512ch, 32×32)
    ...
    "dinov2_vitb14": {
        ...
        1: True,  # blocks.0 - Transformer block 0 (768d, early)
        2: True,   # blocks.1 - Transformer block 1 (768d, early-mid)
        3: True,  # blocks.2 - Transformer block 2 (768d)
        4: True,  # blocks.3 - Transformer block 3 (768d)
        5: True,   # blocks.4 - Transformer block 4 (768d, mid)
        6: True,  # blocks.5 - Transformer block 5 (768d)
        7: True,  # blocks.6 - Transformer block 6 (768d)
        8: True,   # blocks.7 - Transformer block 7 (768d, mid-late)
        9: True,  # blocks.8 - Transformer block 8 (768d)
        10: True, # blocks.9 - Transformer block 9 (768d)
        11: True,  # blocks.10 - Transformer block 10 (768d, late)
        12: True, # blocks.11 - Transformer block 11 (768d)
        13: True,  # norm - Final layer norm (768d, final)
    },
    "vgg19": {
        0: True,   # features.0 - conv1_1 (64ch, 224×224, very early)
        1: True,  # features.2 - conv1_2 (64ch, 224×224)
        2: True,  # features.5 - conv2_1 (128ch, 112×112)
        3: True,   # features.7 - conv2_2 (128ch, 112×112, early-mid)
        4: True,  # features.10 - conv3_1 (256ch, 56×56)
        5: True,  # features.12 - conv3_2 (256ch, 56×56)
        6: True,  # features.14 - conv3_3 (256ch, 56×56)
        7: True,   # features.16 - conv3_4 (256ch, 56×56, mid)
        8: True,  # features.19 - conv4_1 (512ch, 28×28)
        9: True,  # features.21 - conv4_2 (512ch, 28×28)
        10: True, # features.23 - conv4_3 (512ch, 28×28)
        11: True,  # features.25 - conv4_4 (512ch, 28×28, mid-late)
        12: True, # features.28 - conv5_1 (512ch, 14×14)
        13: True, # features.30 - conv5_2 (512ch, 14×14)
        14: True, # features.32 - conv5_3 (512ch, 14×14)
        15: True,  # features.34 - conv5_4 (512ch, 14×14, late)
        16: True, # classifier.0 - FC1 (4096d)
        17: True,  # classifier.3 - FC2 (4096d, final semantic)
    },
    "lpips_vgg": {
        0: True,   # net.slice1.0 - conv1_1 (64ch, 224×224, very early)
        1: True,   # net.slice1.2 - conv1_2 (64ch, 224×224)
        2: True,   # net.slice2.5 - conv2_1 (128ch, 112×112)
        3: True,   # net.slice2.7 - conv2_2 (128ch, 112×112, early-mid)
        4: True,   # net.slice3.10 - conv3_1 (256ch, 56×56)
        5: True,   # net.slice3.12 - conv3_2 (256ch, 56×56)
        6: True,   # net.slice3.14 - conv3_3 (256ch, 56×56)
        7: True,   # net.slice4.17 - conv4_1 (512ch, 28×28, mid)
        8: True,   # net.slice4.19 - conv4_2 (512ch, 28×28)
        9: True,   # net.slice4.21 - conv4_3 (512ch, 28×28)
        10: True,  # net.slice5.24 - conv5_1 (512ch, 14×14, mid-late)
        11: True,  # net.slice5.26 - conv5_2 (512ch, 14×14)
        12: True,  # net.slice5.28 - conv5_3 (512ch, 14×14, late/final)
    },
    "resnet50": {
        7: True,   # layer3 - Residual stage 3 (1024ch, 14×14, mid)
        8: True,   #layer4.0 - Individual bottleneck 0 (2048ch, 7×7)
        12: True, # layer4.1 - Individual bottleneck 1 (2048ch, 7×7)
        13: True, # layer4.2 - Individual bottleneck 2 (2048ch, 7×7)
        14: True, # layer3.5 - Individual bottleneck 5 (1024ch, 14×14)
        15: True, # layer3.4 - Individual bottleneck 4 (1024ch, 14×14)
    },
    "clip_vit_base": {
        0: True,   # vision_model.embeddings - Patch embed (768d, 50 tokens, initial)
        1: True,  # vision_model.encoder.layers.0 - Transformer 0 (768d, early)
        2: True,  # vision_model.encoder.layers.1 - Transformer 1 (768d)
        3: True,   # vision_model.encoder.layers.2 - Transformer 2 (768d, early-mid)
        4: True,  # vision_model.encoder.layers.3 - Transformer 3 (768d)
        5: True,  # vision_model.encoder.layers.4 - Transformer 4 (768d)
        6: True,   # vision_model.encoder.layers.5 - Transformer 5 (768d, mid)
        7: True,  # vision_model.encoder.layers.6 - Transformer 6 (768d)
        8: True,  # vision_model.encoder.layers.7 - Transformer 7 (768d)
        9: True,   # vision_model.encoder.layers.8 - Transformer 8 (768d, mid-late)
        10: True, # vision_model.encoder.layers.9 - Transformer 9 (768d)
        11: True, # vision_model.encoder.layers.10 - Transformer 10 (768d)
